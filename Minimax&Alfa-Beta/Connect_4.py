...
class Joc:
    ...
    def elem_identice(self, lista):
        ...
        """
        simbol_castigator = lista[0]
        aparitii = 1
        for simbol in lista[1:]:
            if simbol_castigator != simbol:
                simbol_castigator = simbol
                aparitii = 1
            else:
                aparitii += 1
                if aparitii == Joc.NR_CONNECT and simbol_castigator != self.GOL:
                    return simbol_castigator
        return False
    ...
    def final(self):
        ...
        for i in range(Joc.NR_LINII):
            linie = [self.matr[i * self.NR_COLOANE + j] for j in range(self.NR_COLOANE)]
            rez = self.elem_identice(linie)
            if rez:
                return rez

        # verificam coloane
        for j in range(self.NR_COLOANE):
            coloana = [self.matr[i * self.NR_COLOANE + j] for i in range(self.NR_LINII)]
            rez = self.elem_identice(coloana)
        ...
        for i in range(self.NR_LINII - self.NR_CONNECT):
            for j in range(self.NR_COLOANE - self.NR_CONNECT):
                diagonala = [self.matr[(i + k) * self.NR_COLOANE + (j + k)] for k in range(self.NR_CONNECT)]
                rez = self.elem_identice(diagonala)
        ...
        for i in range(self.NR_LINII - self.NR_CONNECT):
            for j in range(self.NR_COLOANE - self.NR_CONNECT):
                diagonala = [self.matr[(i + k) * self.NR_COLOANE + (j + self.NR_CONNECT - k - 1)] for k in
                             range(self.NR_CONNECT)]
                rez = self.elem_identice(diagonala)
    ...
    def mutari_joc(self, jucator_opus):
        ...
        for coloana in range(self.NR_COLOANE):
           #coloana plina
            ...
            # cautam prima pozitie libera parcurgand coloanele de jos in sus
            for linie in range(self.NR_LINII - 1, -1, -1):
                if matr_noua[linie * self.NR_COLOANE + coloana] == self.GOL:
                    break

            matr_noua[linie * self.NR_COLOANE + coloana] = jucator_opus
    ...
    def nr_intervale_deschise(self, jucator):
        ...
        for i in range(Joc.NR_LINII):
            linie = [self.matr[i * self.NR_COLOANE + j] for j in range(self.NR_COLOANE)]
            for j in range(len(linie) - Joc.NR_CONNECT):
                if juc_opus in linie[j:j + self.NR_CONNECT]:
                    continue
                rez += 1
        ...
        for i in range(Joc.NR_COLOANE):
            coloana = [self.matr[i * self.NR_COLOANE + j] for i in range(self.NR_LINII)]
            for j in range(len(coloana) - Joc.NR_CONNECT):
                if juc_opus in coloana[j:j + self.NR_CONNECT]:
                    continue
                rez += 1
        ...
        for i in range(self.NR_LINII - self.NR_CONNECT):
            for j in range(self.NR_COLOANE - self.NR_CONNECT):
                diagonala = [self.matr[(i + k) * self.NR_COLOANE + (j + k)] for k in range(self.NR_CONNECT)]
                for k in range(len(diagonala) - Joc.NR_CONNECT):
                    if juc_opus in diagonala[k:k + self.NR_CONNECT]:
                        continue
                    rez += 1
        ...
        if (stare_curenta.j_curent == Joc.JMIN):
            ...
            while not raspuns_valid:
                try:
                    ...
                    # coloana trebuie sa fie in intervalul corect
