...
class Joc:
    ...
    def mutari_joc(self, jucator):
        ...
        #cautam pozitiile goale si le ocupam

        for i in range(len(self.matr)):
            if self.matr[i] == self.__class__.GOL:
                matrice_tabla_noua = list(self.matr)
                matrice_tabla_noua[i] = jucator
                l_mutari.append(Joc(matrice_tabla_noua))
...
class Stare:
    ...
    def __init__(self, tabla_joc, j_curent, adancime, parinte=None, scor=None):
        self.tabla_joc = tabla_joc  # un obiect de tip Joc => â€žtabla_joc.matrâ€
        self.j_curent = j_curent  # simbolul jucatorului curent

        # adancimea in arborele de stari
        #	(scade cu cate o unitate din â€žtataâ€ in â€žfiuâ€)
...
def min_max(stare):
    ...
    # actualizez scorul â€žtataluiâ€ = scorul â€žfiuluiâ€ ales
...
def alpha_beta(alpha, beta, stare):
    ...
    # Calculez toate mutarile posibile din starea curenta (toti â€žfiiiâ€)
    stare.mutari_posibile = stare.mutari_stare()

    if stare.j_curent == Joc.JMAX:
        scor_curent = float('-inf')  # scorul â€žtataluiâ€ de tip MAX

        # pentru fiecare â€žfiuâ€ de tip MIN:
        for mutare in stare.mutari_posibile:
            # calculeaza scorul fiului curent
            stare_noua = alpha_beta(alpha, beta, mutare)

            # incerc sa imbunatatesc (cresc) scorul si alfa
            # â€žtataluiâ€ de tip MAX, folosind scorul fiului curent
        ...
        scor_curent = float('inf')  # scorul â€žtataluiâ€ de tip MIN

        # pentru fiecare â€žfiuâ€ de tip MAX:
        for mutare in stare.mutari_posibile:
            stare_noua = alpha_beta(alpha, beta, mutare)

            # incerc sa imbunatatesc (scad) scorul si beta
            # â€žtataluiâ€ de tip MIN, folosind scorul fiului curent
            if scor_curent > stare_noua.scor:
                stare.stare_aleasa = stare_noua
                scor_curent = stare_noua.scor
